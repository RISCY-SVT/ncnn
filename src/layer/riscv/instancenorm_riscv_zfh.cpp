...
int InstanceNorm_riscv::forward_inplace_fp16sa(Mat& bottom_top_blob, const Option& opt) const
{
    ...
    if (elempack == 1)
    {
        ...
        for (int q = 0; q < c; q++)
        {
            ...
            // mean and var
            __fp16 sum = (__fp16)0;
            __fp16 sqsum = (__fp16)0;
#if __riscv_zvfh && !defined(C906)
            vfloat16m1_t _sum = __riscv_vfmv_s_f_f16m1((__fp16)0, __riscv_vsetvlmax_e32m1());
            vfloat16m1_t _sqsum = __riscv_vfmv_s_f_f16m1((__fp16)0, __riscv_vsetvlmax_e32m1());
            {
                int n = size;
                __fp16* ptr_sum = ptr;
                while (n > 0)
    ...
    if (elempack == packn)
    {
        ...
        for (int q = 0; q < c; q++)
        {
            ...
            vfloat16m1_t _sqsum = __riscv_vfmv_v_f_f16m1((__fp16)0, vl);
